...
from create_grids import add_random, random_array, add_Glider,  starting_array
...
arr = starting_array((n, n))
#add_random(10, arr, 0, 0)
add_random(20, arr, 0, 29)
add_random(20, arr, 20,20)
#add_Glider(0,0,arr)
...
def neighbours_(array, index_):
    ...
    i = index_[0]
    j = index_[1]
    num = (array[(i-1)% n, (j-1)%n], array[(i-1)%n, j], array[(i-1)%n,(j+1)%n], array[i, (j-1)%n], array[i, (j+1)%n], 
        array[(i+1)%n, (j-1)%n], array[(i+1)%n, j], array[(i+1)%n, (j+1)%n])
    return int(sum(num))
...
def main():
    start_game(arr, create_dic(arr,n))
